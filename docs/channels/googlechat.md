...
- `typingIndicator`: `message` (default) posts a `_<Bot> is typing..._` placeholder and edits it into the first reply; `live` posts the same placeholder, then edits it in place with live progress (tool started, thinking, ...); when the turn ends it edits the placeholder to a terminal "done" status and posts the answer as a new message, so Google Chat notifies the user (an in-place edit does not); `none` disables it; `reaction` requires user OAuth and currently falls back to `message` with a logged error under service-account auth. Both `message` and `live` use only the `chat.bot` scope (`spaces.messages.patch`); neither needs user OAuth.
